...
import { getChainHead, getIndexProviderPeerIdFromFilecoinMinerInfo } from '../lib/lotus-client.js'
...
describe('getChainHead', () => {
  it('should return chain head CIDs when RPC call succeeds', async () => {
    ...
    const result = await getChainHead(RPC_URL, RPC_URL, { rpcFn: mockRpcFn })
  ...
  it('should retry on failure and eventually succeed', async () => {
    ...
    const result = await getChainHead(RPC_URL, RPC_URL, { rpcFn: mockRpcFn, maxAttempts: 5 })
  ...
  it('should throw an error when max retries are exceeded', async () => {
    ...
    await assert.rejects(
      async () => {
        await getChainHead(RPC_URL, RPC_URL, { rpcFn: mockRpcFn, maxAttempts: 3 })
      },
      {
  ...
  it('should include the original error as the cause', async () => {
    ...
    await assert.rejects(
      async () => {
        await getChainHead(RPC_URL, RPC_URL, { rpcFn: mockRpcFn, maxAttempts: 1 })
      },
      (err) => {
  ...
  it('correctly fetches real chain head', async () => {
    const result = await getChainHead(RPC_URL, RPC_URL)
...
describe('getIndexProviderPeerIdFromFilecoinMinerInfo', () => {
  it('should return peer ID when all RPC calls succeed', async () => {
    ...
    const result = await getIndexProviderPeerIdFromFilecoinMinerInfo('f0142637', RPC_URL, RPC_URL, {
      rpcFn: mockRpcFn,
    })
  ...
  it('should throw when ChainHead call fails', async () => {
    ...
    await assert.rejects(
      async () => {
        await getIndexProviderPeerIdFromFilecoinMinerInfo('f0142637', RPC_URL, RPC_URL, {
          rpcFn: mockRpcFn,
          maxAttempts: 2,
        })
      },
